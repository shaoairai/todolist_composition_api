...
<template>
  <div class="todoList_items">
    <div
      ...
      v-if="props.todolistData.filter((item) => item.status === false).length !== 0"
    >
      <p>
        {{ props.todolistData.filter((item) => item.status === false).length }}
        個待完成項目
      </p>
    </div>
    ...
    <ul v-else class="todoList_item">
      <template v-for="item in props.todolistData" :key="item.id">
        <li v-if="item.status === false" class="position-relative">
