...
<template>
  <div class="todoList_items">
    <div class="todoList_statistics justify-content-end;" v-if="props.todolistData.length !== 0">
      <p>
        {{ props.todolistData.filter((item) => item.status === false).length }} 個待完成項目<br />
        {{ props.todolistData.filter((item) => item.status === true).length }}
        個已完成項目
      </p>
    </div>

    <p class="text-center" v-if="props.todolistData.length === 0">目前尚無待辦事項</p>

    <ul v-else class="todoList_item">
      <li v-for="item in props.todolistData" :key="item.id" class="position-relative">
